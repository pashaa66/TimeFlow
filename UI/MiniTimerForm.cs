...
public class MiniTimerForm : Form
{
    ...
    private class MiniCard : UserControl
    {
		...
		private static string FormatGoal(int estimatedSeconds)
		{
			...
			var targetDate = baseDate.AddSeconds(estimatedSeconds);
			...
			var afterYears = baseDate.AddYears(years);
			int months = (targetDate.Year - afterYears.Year) * 12 + 
						 (targetDate.Month - afterYears.Month);
			if (targetDate.Day < afterYears.Day)
				months--;

			var afterMonths = afterYears.AddMonths(months);
			int days = (targetDate.Date - afterMonths.Date).Days;
